...
export const registerFeedbackRoutes: FastifyPluginAsync = async (app) => {
    ...
    const service = newFeedbackService(repo,{nowISO: () => new Date().toISOString()}); 
    app.post<{ Body: FeedbackDTO }>("/feedback", {
        schema: {
            body: {
            type: "object",
    ...
    if (e instanceof RepoError) {
      if (e.code === "DB_LOCKED") {
        ...
        return reply.code(503).send({ error: { code: "TEMPORARY_UNAVAILABLE" } });
      }
